...
Json::Document XmlToJson(const Xml::Document& doc) {
  using Json::Node;
  vector<Node> result;
  for (const Xml::Node& x_n : doc.GetRoot().Children()) {
    result.emplace_back(map<string, Node>{
      {"category", Node(x_n.AttributeValue<string>("category"))},
      {"amount", Node(x_n.AttributeValue<int>("amount"))}
    });
  }
...
Xml::Document JsonToXml(const Json::Document& doc, string root_name) {
  ...
  for (const Json::Node& j_n : doc.GetRoot().AsArray()) {
    root.AddChild(Xml::Node("spend", {
      {"category", j_n.AsMap().at("category").AsString()},
      {"amount", to_string(j_n.AsMap().at("amount").AsInt())},
    }));
  }
